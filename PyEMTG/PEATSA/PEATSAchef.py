...
class PEATSAchef(object):
    ...
    def MissedThrust(self,PEATSAorder,prefix = "",MObase = None,SObase = None):
        ...
        for jdx in range(len(RefM.Journeys)):
            ...
            # Most of this code was written to be multi-phase friendly, but the scripted constraints below are not. So throwing an error so user is warned and can handle that.
            if nPhases > 1:
                raise Exception("Scripted constraints in multi-phase journeys is currently broken. See comment below this error for more detail")
                # If using multi-phase journeys but not scripted constraints in those multi-phase journeys, simply comment this conditional statement and error out.
                # If using multi-phase journeys with scripted constraints, search for "COMMENT POINTER FROM MULTI-PHASE EXCEPTION ABOVE." and read the comments there. 
            # If there are more than 9 phases it will break things, so throw an error
            if nPhases > 9:
                raise Exception("Phase number finding code in the decision vector re-creation cant handle more than 9 phases currently")
            ...
            for pdx in range(0,nPhases):      
                ...
                while current_epoch < arrival_epoch:
                    ...
                    MO.forced_post_launch_coast = 0
                    
                    # If needed shorten bounded flight times
                    if MO.Journeys[0].timebounded == 1:
                        MO.Journeys[0].flight_time_bounds[0] -= (current_epoch-phase_start_epoch)
                        MO.Journeys[0].flight_time_bounds[1] -= (current_epoch-phase_start_epoch)
                    ...
                    # Total prop left
                    total_prop_left = RefM.total_electric_propellant_including_margin - used_electric_propellant - propellant_margin_used
                    
                    # Update the tank constraint
                    SO.setGlobalElectricPropellantTankCapacity(total_prop_left)
                    ...
                    if isinstance(PEATSAorder.post_missed_thrust_electric_propellant_margin,str):
                        ...
                        if PEATSAorder.post_missed_thrust_electric_propellant_margin == "same":
                            prop_margin_to_use = MO.electric_propellant_margin
                        else:
                            ...
                            if PEATSAorder.post_missed_thrust_electric_propellant_margin.endswith("pct") or PEATSAorder.post_missed_thrust_electric_propellant_margin.endswith("%"):
                                ...
                                prop_margin_to_use = float(PEATSAorder.post_missed_thrust_electric_propellant_margin.rstrip("pct% ")) / 100.0
                                ...
                                prop_margin_to_use = post_missed_thrust_electric_propellant_margin_kg / (total_prop_left - post_missed_thrust_electric_propellant_margin_kg)
                            else:
                                try:
                                    prop_margin_to_use = float(PEATSAorder.post_missed_thrust_electric_propellant_margin)
                                except:
                    ...
                    MO.electric_propellant_margin = prop_margin_to_use
                           
                    # Make sure we have some propellant left
                    if total_prop_left < 0:
                        raise Exception("Negative propellant remaining. Something went wrong. Maybe using a fixed post margin kg but also removing propellant margin before the MTE, leaving negative left")
                    ...
                    # Removed RHS journeys
                    removed_rhs_journeys = False
                    
                    # Set some flags to be overriden if need be
                    new_bc = None
                    new_bc_tolerance = None
                    new_bc_epoch_tol = None
                    
                    # Remove subsequent journeys if the user asked for that
                    if len(PEATSAorder.journey_boundaries):
                        
                        # Loop through all of the journey boundaries
                        for jb in PEATSAorder.journey_boundaries:
                            
                            # Check if this journey boundary is greater than the current journey 
                            if jb[0] >= jdx:
                                # It is. We are going to end the mission here.
                                
                                # Store a flag
                                removed_rhs_journeys = True
                                
                                # Pop the future journeys
                                
                                # Create a list
                                indices_to_pop = []
                                for jdx2 in range(jb[0],len(RefMO.Journeys)-1):
                                    indices_to_pop.append(jdx2-jdx+1)
                                
                                # Reverse the list
                                indices_to_pop.reverse()
                                
                                # do the pop
                                for jdx2 in indices_to_pop:
                                    MO.Journeys.pop(jdx2)
                                    MO.number_of_journeys -= 1
                                    
                                # Save the last journey
                                last_journey = jb[0]
                                
                                # Save the new boundary condition rule
                                new_bc = jb[2]
                                
                                # If we have tolerances add them
                                if len(jb) > 3:
                                    new_bc_tolerance = jb[3]
                                else:
                                    new_bc_tolerance = None
                                
                                # If we are locking vinf magnitude, we need a list
                                if new_bc == 2:
                                    vinf = []
                                
                                if jb[1] > 0:
                                    # Find time remaining until journey boundary
                                    timeremaining = remaining_phase_flight_time
                                
                                    for key in all_phase_flight_times.keys():
                                    
                                        jdx2 = int(key.split("p")[0].split("j")[1])
                                        pdx2 = int(key.split("p")[1])
                                    
                                        if jdx2 == jdx and pdx2 > pdx:
                                            timeremaining += all_phase_flight_times[key]
                                        elif jdx2 > jdx and jdx2 <= jb[0]:
                                            timeremaining += all_phase_flight_times[key]
                                    
                                    MO.Journeys[-1].timebounded = 2
                                    MO.Journeys[-1].arrival_date_bounds[0] = current_epoch + timeremaining - 2400000.5 - jb[1]/2
                                    MO.Journeys[-1].arrival_date_bounds[1] = current_epoch + timeremaining - 2400000.5 + jb[1]/2                                
                                ...
                                for local_decision_vector_index in range(phase_start_decision_vector_index,len(RefM.Xdescriptions)):
                                    ...
                                    jdx2 = int(RefM.Xdescriptions[local_decision_vector_index].split("p")[0].split("j")[1])
                                    
                                    if jdx2 > jb[0]:
                                                     
                                        # Check if this is the virtual electric propellant
                                        if "virtual electric propellant" in RefM.Xdescriptions[local_decision_vector_index] and "Step" not in RefM.Xdescriptions[local_decision_vector_index] and "step" not in RefM.Xdescriptions[local_decision_vector_index]:
                                            prop_2_remove += RefM.DecisionVector[local_decision_vector_index]
                                
                                
                                marginprop = MO.electric_propellant_margin * total_prop_left / (1+MO.electric_propellant_margin)
                                usable_prop_left = total_prop_left - marginprop
                                
                                usable_prop_to_boundary = usable_prop_left - prop_2_remove
                
                                SO.setGlobalElectricPropellantTankCapacity(usable_prop_to_boundary)
                                
                                MO.electric_propellant_margin = 0.0
                                
                                if usable_prop_to_boundary < 0:
                                    raise Exception("Negative propellant remaining. Something went wrong")
                                                                
                                # Done looping through journey_boundaries
                                break
                    ...
                    for local_decision_vector_index in range(phase_end_decision_vector_index,len(RefM.DecisionVector)):
                        ...
                        # Check if we've removed RHS journeys
                        if removed_rhs_journeys:
                            if journey_number > last_journey:
                                # We have removed this joureny
                                continue
                        ...
                        if journey_number == jdx:
                            ...
                            # In case we need it, store the original journey number
                            og_journey_number = journey_number
                        ...
                        # Check if the journey of this variable matches the one we are iterating through
                        if removed_rhs_journeys and og_journey_number == last_journey and new_bc == 1 and "PeriapseFlybyIn" in new_description:
                            # Need to add constraints
                            # Lock r
                            if "event left state r" in new_description:
                                
                                # Get the nominal value from the decision vector
                                nominal_value = RefM.DecisionVector[local_decision_vector_index]
                                
                                # Create teh new constraint string
                                new_constraint = "p" + str(phase_number) + "_arrival_distanceconstraint_cb_" + str(nominal_value - new_bc_tolerance[0]/2) + "km_" + str(nominal_value + new_bc_tolerance[0]/2) + "km"
                                
                                MO.Journeys[journey_number].BoundaryConstraintDefinitions.append(new_constraint)
                                
                            # Lock v
                            if new_description.endswith("event left state v"):
                                
                                # Get the nominal value from the decision vector
                                nominal_value = RefM.DecisionVector[local_decision_vector_index]
                                
                                # Create teh new constraint string
                                new_constraint = "p" + str(phase_number) + "_arrival_VelocityMagnitude_" + str(nominal_value - new_bc_tolerance[1]/2) + "_" + str(nominal_value + new_bc_tolerance[1]/2) + "_ICRF"
                                
                                MO.Journeys[journey_number].BoundaryConstraintDefinitions.append(new_constraint)
                                
                            # Lock RA
                            if "event left state RA" in new_description:
                                import math
                                
                                # Get the nominal value from the decision vector
                                nominal_value = RefM.DecisionVector[local_decision_vector_index] * 180.0/math.pi
                                
                                # Create teh new constraint string
                                new_constraint = "p" + str(phase_number) + "_arrival_longitude_" + str(nominal_value - new_bc_tolerance[2]/2) + "_" + str(nominal_value + new_bc_tolerance[2]/2) + "_ICRF"
                                
                                MO.Journeys[journey_number].BoundaryConstraintDefinitions.append(new_constraint)
                            
                            # # Lock vRA
                            # This doesnt appear to work in ICRF
                            # if "event left state vRA" in new_description:
                            #     import math
                            #
                            #     # Get the nominal value from the decision vector
                            #     nominal_value = RefM.DecisionVector[local_decision_vector_index] * 180.0/math.pi
                            #
                            #     # Create teh new constraint string
                            #     new_constraint = "p" + str(phase_number) + "_arrival_VelocitySphericalAzimuth_" + str(nominal_value - new_bc_tolerance[3]/2) + "_" + str(nominal_value + new_bc_tolerance[3]/2) + "_ICRF"
                            #
                            #     MO.Journeys[journey_number].BoundaryConstraintDefinitions.append(new_constraint)
                            
                            # Lock vDEC
                            if "event left state vDEC" in new_description:
                                import math
                                ...
                                # Create teh new constraint string
                                new_constraint = "p" + str(phase_number) + "_arrival_VelocityDeclination_" + str(nominal_value - new_bc_tolerance[4]/2) + "_" + str(nominal_value + new_bc_tolerance[4]/2) + "_ICRF"
                                
                                MO.Journeys[journey_number].BoundaryConstraintDefinitions.append(new_constraint)
                        elif removed_rhs_journeys and og_journey_number == last_journey and new_bc == 2 and "EphemerisPeggedIntercept" in new_description:
                            # Need to add constraints
                            import math
                            
                            # Lock v inf x
                            if "V_infinity_x" in new_description:
                                vinf.append(RefM.DecisionVector[local_decision_vector_index])
                            # Lock v inf y
                            if "V_infinity_y" in new_description:
                                vinf.append(RefM.DecisionVector[local_decision_vector_index])
                            # Lock v inf z
                            if "V_infinity_z" in new_description:
                                vinf.append(RefM.DecisionVector[local_decision_vector_index])
                                
                                if len(vinf) != 3:
                                    raise Exception("Got to v infinity z, but dont have the full vector?!?")
                                
                                # Calculate total vinfinity
                                vinfmag = math.sqrt(vinf[0]**2 + vinf[1]**2 + vinf[2]**2)

                                # Update the constraint
                                MO.Journeys[journey_number].final_velocity[0] = vinfmag - new_bc_tolerance/2
                                MO.Journeys[journey_number].final_velocity[1] = vinfmag + new_bc_tolerance/2
                    ...
                    # COMMENT POINTER FROM MULTI-PHASE EXCEPTION ABOVE. 
                    # Code block below will update scripted constraints in a multi-phase friendly way. HOWEVER, 
                    # this code does not work if using journey_boundaries!! Currently no easy work around 
                    # for using scripted constraints in a multi-phase journey AND journey_boundaries. Sorry. Jeremy was busy 
                    # and didnt have time to build it. 
                    # 
                    # If using scripted constraints in a multi-phase journey, and NOT using journey_boundaries, then 
                    # you can simply switch this flag to True
                    if False:
                        # Create a list of the phase distance constraints that will be in the new problem
                        new_boundary_constraints = []
                        ...
                        for constraint in MO.ManeuverConstraintDefinitions:
                            ...
                            if constraint_journey < jdx:
                                ...
                                if constraint_journey == jdx and phase_number < pdx:
                                    ...
                                    if constraint_journey != jdx or phase_number != pdx:
                                        # We do not need to add it
                                        continue
                                    elif "b" in constraint_prefix and int(constraint_prefix.split("b")[1]) < first_control_step:
                                        continue
                                    else:
                                        # We do need to add it

                                        if "b" in constraint_prefix:
                                            burn_number = int(constraint_prefix.split("b")[1])
                                        ...
                                        if "b" in constraint_prefix:
                                            ...
                                            new_maneuver_constraints.append("j" + str(constraint_journey) + "p" + str(phase_number) + constraint.split(constraint_prefix)[1])

                        # Overwrite the new phase constraint list onto the old list
                        MO.ManeuverConstraintDefinitions = new_maneuver_constraints
                    else:
                        # To update the journeys in the scripted constraints, reassemble
