    ...
    namespace Astrodynamics {
        ...
        IntegratedFixedStepPropagator::IntegratedFixedStepPropagator(const size_t & numStates_in,
                                                                     ...
                                                                     IntegratedPropagator(numStates_in, 
                                                                                          STM_size_in)
        {
            this->propagation_time_history.reserve(1000);
        }
                                                      ...
                                                      const bool & STM_needed)
        {
            ...
            {
				// Create a simple double vector for the state
				std::vector<double> current_state(8);
				// Put the DoubleType vector into the current state
				for (int idx = 0; idx < 8; idx++)
					current_state[idx] = this->state_left(idx) _GETVALUE;
				this->propagation_state_history.push_back(current_state);
			}
            ...
            if (propagation_span_remaining > 0.0)
            {
                while (propagation_span_remaining > 0.0)
                {
                    ...
                    if (this->store_propagation_history)
                    {
                        this->propagation_time_history.push_back(this->state_right(this->index_of_epoch_in_state_vec) _GETVALUE - this->state_left(this->index_of_epoch_in_state_vec) _GETVALUE);
						// Create a simple double vector for the state
						std::vector<double> current_state(8);
						// Put the DoubleType vector into the current state
						for (int idx = 0; idx < 8; idx++)
							current_state[idx] = this->state_right(idx) _GETVALUE;
						this->propagation_state_history.push_back(current_state);
                    }
