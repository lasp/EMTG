        ...
        for (size_t Xindex2 = 0; Xindex2 < this->total_number_of_NLP_parameters; ++Xindex2)
        {
            ...
            for (size_t trialXindex2 = 0; trialXindex2 < processed_initial_guess.size(); ++trialXindex2)
            {
                ...
                if (match)
                {
					...
					else if (this->options.Journeys[thisJourney].freeze_maneuver_variables.size())
					{	
						// Store a flag to determine if we need to freeze it or not
						bool maneuver_in_list = false;
						
						// Store an index to the burn index, initialized to a non real value
						int current_burn_idx = -1;
							
						// check if this is an nDSMs input and extract the burn number if so. Example decision variable string: p0MGAnDSMsForwardSubPhase0: DSM x component
						if (this->Xdescriptions[Xindex2].find("DSM") != std::string::npos && this->Xdescriptions[Xindex2].find("component") != std::string::npos)
						{
		                    std::vector<std::string> DescriptionCell2;
							// Extract the string before the colon. Remaining string: p0MGAnDSMsForwardSubPhase0
		                    boost::split(DescriptionCell2, this->Xdescriptions[Xindex2], boost::is_any_of(":"), boost::token_compress_on);
		                    std::vector<std::string> DescriptionCell3;
							// Extract the string after the 'e' in Phase. Remaining string: '0'
		                    boost::split(DescriptionCell3, DescriptionCell2[0], boost::is_any_of("e"), boost::token_compress_on);
							// Convert the string to an integer
							current_burn_idx = std::stoi(DescriptionCell3[DescriptionCell3.size()-1]);
						}
						// check if this is an PSFB input and extract the burn number if so. Example desision variable string: p0PSFB_Step0: substep0 u_x
						else if (this->Xdescriptions[Xindex2].find("PSFB") != std::string::npos && this->Xdescriptions[Xindex2].find("substep") != std::string::npos)
						{
		                    std::vector<std::string> DescriptionCell2;
							// Extract the string before the colon. Remaining string: p0PSFB_Step0
		                    boost::split(DescriptionCell2, this->Xdescriptions[Xindex2], boost::is_any_of(":"), boost::token_compress_on);
		                    std::vector<std::string> DescriptionCell3;
							// Split the string by 'p's
		                    boost::split(DescriptionCell3, DescriptionCell2[0], boost::is_any_of("p"), boost::token_compress_on);
							// Convert the string to an integer by grabbing the last entry in the list
							current_burn_idx = std::stoi(DescriptionCell3[DescriptionCell3.size()-1]);
						}
						// check if this is an FBLT input and extract the burn number if so. Example desision variable string: p0FBLT: step 13 u_z or p0MGALT: step 17 u_z
						else if ((this->Xdescriptions[Xindex2].find("FBLT") != std::string::npos || this->Xdescriptions[Xindex2].find("MGALT") != std::string::npos) && this->Xdescriptions[Xindex2].find("step") != std::string::npos)
						{
		                    std::vector<std::string> DescriptionCell2;
							// Extract the string before the colon. Remaining string: step 13 u_z
		                    boost::split(DescriptionCell2, this->Xdescriptions[Xindex2], boost::is_any_of(":"), boost::token_compress_on);
		                    std::vector<std::string> DescriptionCell3;
							// Split the string by spaces: ["","step","13","u_z"]
		                    boost::split(DescriptionCell3, DescriptionCell2[1], boost::is_any_of(" "), boost::token_compress_on);
							// Convert the string to an integer by grabbing the second entry in the list
							current_burn_idx = std::stoi(DescriptionCell3[2]);
						}
						
						// Check if this maneuver is in the list to freeze
						for (size_t frozen_indices = 0; frozen_indices < this->options.Journeys[thisJourney].freeze_maneuver_variables.size(); frozen_indices++)
							if (this->options.Journeys[thisJourney].freeze_maneuver_variables[frozen_indices] == current_burn_idx)
							{
								maneuver_in_list = true;
								break;
							}
					
						// We will now know if the manuever is in list or not. If it is, freeze its bounds
						if (maneuver_in_list)
						{
	                        this->Xlowerbounds[trialXindex2] = newX[Xindex2] - math::SMALL;
	                        this->Xupperbounds[trialXindex2] = newX[Xindex2] + math::SMALL;
