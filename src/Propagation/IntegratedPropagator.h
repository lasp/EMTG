...
namespace EMTG {
    namespace Astrodynamics {
        ...
        public:
            ...
            // methods
            virtual IntegratedPropagator* clone() const override = 0;
            ...
            virtual inline std::vector<double> getPropagationTimeHistory() const override  
            { 
                if (this->propagation_time_history.size() > 0)
                {
                    return this->propagation_time_history;
                }
                else
                {
            ...
            virtual inline std::vector< std::vector<double> > getPropagationStateHistory() const override  
            { 
                if (this->propagation_state_history.size() > 0)
                {
                    return this->propagation_state_history;
                }
                else
                {
                    throw std::runtime_error("propagation history not stored...set using PropagatorBase::setStorePropagationHistory(bool)");
            ...
            virtual void propagate(const doubleType & propagation_span, const bool & needSTM) override = 0;
            virtual void propagate(const doubleType & propagation_span, const math::Matrix <doubleType> & control, const bool & needSTM) override = 0;
