...
class PEATSAmenu(object):
    def __init__(self,options_file = None):
        ...
        self.init_var(
            ...
            "False",
       "# Rather than run a global optimization, would you like to run post missed thrust cases on a limited\n\
        # problem. Enter a list of tuples. Each tuple has the form:\n\
        # (journey_index,lock_epoch,boundary_condition,tolerance,lock_epoch)\n\
        # journey index is the index after which to create a new boundary. All missed thrust cases before this will stop after this journey.\n\
        # NOTE: journey_index must be in order in the journey_boundaries list!\n\
        # lock_epoch = 0          No changes to boundary epoch.\n\
        # lock_epoch > 0          Lock epoch to the reference trajectory epoch to +/- lock_epoch days\n\
        # boundary_condition = 0, keep the boundary condition the same as in the reference mission. Tolerance not used\n\
        # boundary_condition = 1, Original Journey is a Periapse boundary condition. Lock r, v, RA, vRA, vDEC. Tolerance is a list\n\
        #                         corresponding to each locked value, in order, with units of km, km/s, deg, deg, deg. Example [1,.001,.5,.5,.5]\n\
        #                         DEC cant be locked currently because its not a scriptable constraint in ICRF.\n\
        # boundary_condition = 2, Original Journey is an Ephemeris pegged Intercept. Lock v inf magnitude to tolerance specified.\n\
        # NOTE: Propellant mass will always be locked to propellant usage in reference trajectory + any released margin"
        )
