...
namespace EMTG
{
    ...
    void problem::output_problem_bounds_and_descriptions(std::string filestring)
    {
        ...
        outputfile << "Xindex, Description, Lowerbound, Upperbound, ScaleFactor, Value, LowerBoundMargin, UpperBoundMargin, WorstMargin" << std::endl;
        for (size_t k = 0; k < Xdescriptions.size(); ++k)
		{
            outputfile << k << "," << Xdescriptions[k] << "," << Xlowerbounds[k] << "," << Xupperbounds[k] << "," << X_scale_factors[k] << "," << X[k] _GETVALUE<< ","<< X[k] _GETVALUE - Xlowerbounds[k] << "," << Xupperbounds[k] - X[k] _GETVALUE << ",";
			outputfile << ((X[k] _GETVALUE - Xlowerbounds[k] < Xupperbounds[k] - X[k] _GETVALUE) ? (X[k] _GETVALUE - Xlowerbounds[k])/X_scale_factors[k] : (Xupperbounds[k] - X[k] _GETVALUE)/X_scale_factors[k] );
			outputfile << std::endl;
		}

        outputfile << "Findex, Description, Lowerbound, Upperbound, , Value, LowerBoundMargin, UpperBoundMargin, WorstMargin" << std::endl;
        for (size_t k = 0; k < Fdescriptions.size(); ++k)
		{
            outputfile << k << "," << Fdescriptions[k] << "," << Flowerbounds[k] << "," << Fupperbounds[k] << "," << "," << F[k] _GETVALUE<< ","<< F[k] _GETVALUE - Flowerbounds[k] << "," << Fupperbounds[k] - F[k] _GETVALUE << ",";
			outputfile << ((F[k] _GETVALUE - Flowerbounds[k] < Fupperbounds[k] - F[k] _GETVALUE) ? F[k] _GETVALUE - Flowerbounds[k] : Fupperbounds[k] - F[k] _GETVALUE);
			outputfile << std::endl;
		}
        outputfile.close();
    }//end output_problem_bounds_and_descriptions()
